...
6. Se calcula la desviación estándar de las sondas, seleccionándose las 10.000 sondas con mayor variabilidad.
...
sondas_en_GSE <- GSE42409[GSE42409$ID %in% rownames(data), ]
...
```{r supresion_muestras, eval=FALSE}
...
## Selección de las 10.000 sondas de mayor desviación estándar
...
### Selección de las 10000 sondas de mayor desviación estándar
...
sondas_10000 <- selecciona_sondas(data_sondas_dep, 10000)
save(sondas_10000, file="G:/TFM UOC/datos/Clasificador_variabilidad/sondas_10000.Rda")
...
load(file="G:/TFM UOC/datos/Clasificador_variabilidad/sondas_10000.Rda")

df <- rowRanges(sondas_10000)$sd 
df <- data.frame(sd = df)
...
ggplot(data = df , aes(x=sd)) +
  ...
  stat_function(fun=dnorm, colour="#0C3D7D9F", args=list(mean=mean(df$sd), sd = sd(df$sd))) +
...
load("G:/TFM UOC/datos/Clasificador_variabilidad/sondas_10000.Rda")

set.seed(123)
etiqueta <- sondas_10000$label

in_train <- createDataPartition(etiqueta, p=0.75, list=FALSE) %>% as.vector()

train <- sondas_10000[ , in_train]
test <- sondas_10000[ , -in_train] 
train
test
