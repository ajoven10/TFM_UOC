---
title: "Clasificador diseasecodes TCGA. Red neuronal con sondas diferencialmente metiladas Tumor vs. Control"
...
Este trabajo se inspira en el artículo [@capper2018dna] que propone un clasificador Machine Learning para la determinación del tipo de tumor del SNC a partir de los datos de metilación (betavalues) de las 10.000 sondas del array Illumina 450k que presentan una mayor variabilidad. También ha servido de guía el planteamiento descrito en [@maros2020machine] que expone una aplicación general de esa metodología.
...
4. Se desglosan las muestras en los grupos train y test (75% y 25%) utilizando la librería *caret* [@max2017caret].
5. Se seleccionan las sondas de acuerdo al criterio de metilación diferencial: p-value ajustado con BH para la diferencia de medias entre los grupos contrastados> 0.01. Se ha contrastado acda tumor contra el conjunto d emuestrasde control con la función TCGAanalize que emplea el test de Wilcoxon. Se han seleccionadop las sondas que cumplían el criteriode False Discovery Rate ajustado con el criterio BH menor que 0.01 y no se ha considerado un valor mínimo para la diferencia en las medias de los valores betas entre ambos grupos. El subset de sondas se obtiene por la intersección  de los subconjuntos de sondas diferenciales obtenido con los 33 contrastes tumor vs Control.
...
En este trabajo usaré solo las muestras de códigos 01 Primary Solid Tumor (TP), 03 Primary Blood Derived Cancer - Peripheral Blood (TB) y 11 Solid Tissus Normal (NT).
...
# Selección de las  sondas diferencialmente metiladas

## Cálculo de la metilación diferencial
...
```{r supresion_sondas, cahe=TRUE}

load("G:/TFM UOC/datos/data_1524sondas.Rda")
data <- data_1524sondas
...
## Gráfico previo Rtsne para las muestras de control exclusivamente

```{r rtnse_control, cache=TRUE}
...
Las muestras de control, al proceder de tejidos dispares presentan un alto grado de dispersión en este análisis de similiritud.
...
El histograma de valores betas por tumor (solo consideradas las 10.000 sondas seleccionadas) muestra diferencias significativas entre los diferentes tumores, sin embrago el grupo de control representado en el séptimo lugar no se identifica o discrimina claramente con respecto al resto.


# El clasificador RED neuronal

## Selección de muestras con código tumor primario y muestras de control

Se seleccionan en los grupos train y test las muestras cuyos códigos son 01 Primary Solid Tumor (TP), 03 Primary Blood Derived Cancer - Peripheral Blood (TB) y 11 Solid Tissus Normal (NT).

```{r muestras_tumor_primario, cache=TRUE}
...
## Predicción con lo valores test utilizando el modelo con epoch 70 seleccionado
