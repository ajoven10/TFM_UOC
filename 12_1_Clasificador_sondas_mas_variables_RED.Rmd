---
title: "Clasificador diseasecodes TCGA. Red neuronal con sondas de mayor variabilidad"
...
5. Se calcula la desviación estándar de las sondas en el grupo *train* de muestras, seleccionándose las 10.000 sondas con mayor variabilidad.
...
# Selección de las 10.000 sondas con mayor variabilidad
...
```{r desglose_train_test, eval =FALSE}
...
## Selección de las 10.000 sondas de mayor desviación estándar del grupo train

### Selección

```{r seleccion_sondas, eval= FALSE}
...
df <- data.frame(betas = train_data[ label =="BRCA" , sample(1:10000, 1)], label = l )
...
```{r datos_random, cache=TRUE, warning=FALSE, message=FALSE}
...
## Exclusión de muestras que no son tumorel primarios o control

```{r tumor_random, cache=TRUE}
